...
async def handle_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- DELETE DATA LOGIC ---
        ...
        # FIXED: Handles spaces in chapter names
...
async def handle_file_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id) and context.user_data.get('adm_mode') == 'active' and doc.mime_type == 'text/plain':
        ...
        for line in text_data.split('\n'):
            ...
            # Format: Q | A | B | C | D | CorrectIndex (1-4)
            if len(parts) >= 6:
                try:
                    q_text = parts[0]
                    opts = parts[1:5] # Take next 4
                    correct_idx = int(parts[-1]) - 1 # 1-based to 0-based
...
if __name__ == '__main__':
    ...
    if not TOKEN: print("❌ TOKEN MISSING")
        ...
        # Handle both JSON (backup) and TXT (questions)
