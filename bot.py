...
async def handle_menus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('set_grp_mode_'):
        ...
        # Stop any existing Repeating Jobs
        current_jobs = context.job_queue.get_jobs_by_name(gid)
        for job in current_jobs: job.schedule_removal()
        
        # Start the SMART LOOP (Run Once -> Then Self-Schedule)
        # This allows dynamic interval changes without re-registering
...
async def auto_group_job_smart(context):
    """
    Self-scheduling job. Runs once -> Checks current interval -> Schedules next run.
    This enables 'Live Interval Changes' without restarting the job.
    """
    ...
    if last_msg_id:
        ...
        except Exception: pass # Ignore if already deleted or permission issue
    ...
    if not all_qs:
        # Stop loop if no questions found to avoid spamming logs
        return 
    ...
    try:
        ...
        # Save Message ID for deletion next time
        db["groups"][chat_id]["last_msg_id"] = msg.message_id
        save_db(db) # Save DB to persist msg_id across restarts
        
    except Exception as e:
        logging.error(f"Group Poll Error {chat_id}: {e}")

    # 5. SCHEDULE NEXT RUN (Recursion with current DB interval)
    # Fetch fresh interval from DB every time!
