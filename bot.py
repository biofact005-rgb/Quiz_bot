...
# --- LIVE VERIFICATION CONFIG ---
# बॉट को यहाँ ADMIN होना जरूरी है
MAIN_CHANNEL_ID = "@errorkid_05"  # (Better: Use -100 ID if you have it)
...
async def check_membership(chat_id, user_id, context):
    ...
    Returns: True (Member/Allowed), False (Not Member/Left)
    """
    try:
        # Check Member Status
        ...
        # Case 1: Bot is not admin in channel
        # Case 2: Chat ID/Username is wrong
        logging.error(f"⚠️ Verification Failed for {chat_id}: {e}")
        # IMPORTANT: If bot fails to check, we allow user (Fail-Open) 
        # so the bot doesn't stop working entirely.
        return True 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # 1. VERIFICATION CHECK (MAIN CHANNEL)
        # Using MAIN_CHANNEL_ID (Make sure bot is admin there!)
        ...
        # Fallback reply so bot is never silent
        await update.message.reply_text("⚠️ Error starting bot. Please try again later.")
...
async def handle_menus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'recheck_main':
        if await check_membership(MAIN_CHANNEL_ID, user_id, context):
            await show_main_menu(update, context)
        else:
    ...
    if data == 'gate_bseb' or data == 'recheck_bseb':
        if await check_membership(ID_BSEB_GROUP, user_id, context):
    ...
    if data == 'gate_neet' or data == 'recheck_neet':
        if await check_membership(ID_NEET_GROUP, user_id, context):
