...
# Make sure Bot is Admin in these Groups
IDS = {
    "MAIN": "@errorkid_05", 
    "BSEB": -1002398369446, 
    "NEET": -1002792641130
...
async def check_membership(chat_id, user_id, context):
    """
    Returns True ONLY if user is a member/admin/creator.
    """
    try:
        # Use context.bot (Fix applied here)
        member = await context.bot.get_chat_member(chat_id=chat_id, user_id=user_id)
        if member.status in ['left', 'kicked', 'banned']:
            return False
        return True
    except BadRequest as e:
        logging.error(f"⚠️ VERIFICATION ERROR: {e} | Chat: {chat_id}")
        # Fail Open: If bot fails to check (e.g. not admin), let user in to avoid blocking.
        return True 
    except Exception as e:
        logging.error(f"Unknown Verify Error: {e}")
        return True
...
async def debug_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for name, cid in IDS.items():
        try:
            ...
            msg += f"✅ **{name}:** Connected ({chat.title})\n"
        except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update.effective_chat.type == 'private':
            is_joined = await check_membership(IDS["MAIN"], user.id, context)
            if not is_joined:
                await send_force_join_msg(update, IDS["MAIN"], LINKS["MAIN"], "recheck_main")
                return
...
async def handle_menus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # IMPORTANT: Passing 'context' here fixed the error
    if data == 'gate_bseb': await check_gate(query, context, IDS["BSEB"], LINKS["BSEB"], open_bseb_menu, 'recheck_bseb')
    if data == 'gate_neet': await check_gate(query, context, IDS["NEET"], LINKS["NEET"], open_neet_menu, 'recheck_neet')
    
    if data == 'recheck_bseb': await check_gate(query, context, IDS["BSEB"], LINKS["BSEB"], open_bseb_menu, 'recheck_bseb', alert=True)
    if data == 'recheck_neet': await check_gate(query, context, IDS["NEET"], LINKS["NEET"], open_neet_menu, 'recheck_neet', alert=True)
    ...
    # --- IMPROVE FLOW ---
        ...
        await safe_edit_message(query, "🎯 **Select Category:**", InlineKeyboardMarkup(btns))
        return
    ...
    # Quiz Selection
        ...
        if is_admin(user_id):
            ...
            await safe_edit_message(query, f"⚙️ **Admin Options:**\n`{chap}`", InlineKeyboardMarkup(btns))
        else:
            await ask_time(query)
...
async def check_gate(query, context, gid, link, success_cb, cb_data, alert=False):
    # Pass 'context' to check_membership correctly
    if await check_membership(gid, query.from_user.id, context):
        await success_cb(query)
    else:
        if alert: await query.answer("❌ You must join the group first!", show_alert=True)
...
async def send_force_join_msg(update, name, link, cb):
    ...
    text = f"🚫 **Access Denied!**\n\nYou must join {esc(name)} to use this bot."
...
async def register_group(update, context):
    ...
    btns = [
        [InlineKeyboardButton("📚 BSEB (All Chapters)", callback_data=f'set_grp_mode_BSEB_{chat_id}')],
        [InlineKeyboardButton("🩺 NEET (All Chapters)", callback_data=f'set_grp_mode_NEET_{chat_id}')]
    ]
    await update.message.reply_text("⚙️ **Select Quiz Category:**\n_Clicking will START the quiz immediately!_", reply_markup=InlineKeyboardMarkup(btns))

async def auto_group_job_smart(context):
...
if __name__ == '__main__':
    ...
    if not TOKEN: print("❌ TOKEN MISSING")
        ...
        app.add_handler(CallbackQueryHandler(handle_admin, pattern='^menu_admin|adm_|mng_grp_|tgl_pwr_|ask_cust_int|add_admin|get_backup|restore_prompt|del_|confirm_del'))
        ...
        app.add_handler(CallbackQueryHandler(handle_menus, pattern='^main_menu|gate_|verify_|recheck_|menu_|sel_sub_|mode_|tgl_|confirm_mix|sng_|time_|count_|show_help|set_grp_mode_|imp_|proceed_time'))
