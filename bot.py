...
# Bot MUST be Admin in these chats
...
async def check_membership(chat_id, user_id, context):
    """Checks membership status. Returns True if member."""
    try:
        ...
        # If bot is not admin or chat not found, allow access (Fail Open) to prevent soft-lock
        # Ideally, make bot admin to fix this.
        return False
    except Exception as e:
...
async def show_main_menu(update, context):
    ...
    # NOTE: Removed Markdown form fname to prevent "Can't parse entities" crash
    intro_text = (
        f"━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"✨  **W E L C O M E** ✨\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
...
async def handle_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_admin':
        ...
        # FIXED RECURSION ERROR HERE
        gid = data.split('_')[2]
        curr = db["groups"][gid].get("active", True)
        db["groups"][gid]["active"] = not curr
        save_db(db)
        
        # Redraw menu directly instead of recursive call
        grp = db["groups"][gid]
        status = "✅ ON" if grp.get("active", True) else "🔴 OFF"
        interval = grp.get("interval", 600)
