...
async def handle_menus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('set_grp_mode_'):
        parts = data.split('_')
        mode = parts[3] 
        gid = str(parts[4]) # Ensure String ID
        ...
        # Start New Job
        context.job_queue.run_repeating(auto_group_job, interval=interval, first=2, data=gid, name=gid)
...
async def auto_group_job(context):
    ...
    if not all_qs:
        # SEND ERROR MESSAGE (Fail Loudly)
        try: await context.bot.send_message(chat_id, f"❌ **No questions found in Database for {mode}!**\nPlease add questions via Admin Panel.")
        except: pass
        # Stop job to prevent spamming error
        current_jobs = context.job_queue.get_jobs_by_name(chat_id)
        for job in current_jobs: job.schedule_removal()
        return 
