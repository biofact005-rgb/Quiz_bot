...
def get_random_questions(category, subject, chapters_list, count=10):
    all_q = []
    # If chapters_list is empty or None, fetch ALL chapters
...
async def handle_menus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_improve':
        ...
        if not btns:
            await query.answer("🎉 No mistakes found in this category!", show_alert=True)
            return

        btns.append([InlineKeyboardButton("⬅️ Back", callback_data='menu_improve')])
        await safe_edit_message(query, f"🎯 **{cat} - Select Subject:**\n_Only subjects with mistakes are shown._", InlineKeyboardMarkup(btns))
        return
        ...
        # Start Improve Quiz directly
        questions = get_mistake_questions(user_id, cat, sub)
        if not questions:
            await query.answer("✅ You have improved all questions!", show_alert=True)
            await handle_menus(update, context) # Reload menu
            return

        context.user_data['stop_quiz'] = False
        await safe_edit_message(query, f"🚀 **Improving {sub}!**\nQs: {len(questions)}\n\n_Correct answers will be removed from mistakes._", None)
...
async def run_quiz_sequence(context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, q in enumerate(job['q']):
        ...
        try:
            ...
            # Store full question data to handle mistakes later
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if answerFuture exists (for Instant Next)
    ...
    if poll_id in db.get("current_polls", {}):
        ...
        if q_data:
            ...
            if selected_option != correct_id:
                # WRONG ANSWER: Add to mistakes if not present
                if q_data not in mistakes_list:
                    mistakes_list.append(q_data)
            
            else:
                # RIGHT ANSWER: 
                # If mode is 'improve', remove from mistakes
                if mode == 'improve':
...
async def auto_group_job_smart(context):
    ...
    mode = grp.get("mode", "BSEB")
    
    last_msg_id = grp.get("last_msg_id")
    if last_msg_id:
        try:
    ...
    try:
        ...
        db["groups"][chat_id]["last_msg_id"] = msg.message_id
        save_db(db)
        
    except Exception as e:
        logging.error(f"Group Poll Error {chat_id}: {e}")

    current_interval = db["groups"][chat_id].get("interval", 600)
    context.job_queue.run_once(auto_group_job_smart, current_interval, data=chat_id, name=chat_id)
...
if __name__ == '__main__':
    ...
    if not TOKEN: print("❌ TOKEN MISSING")
        ...
        app.add_handler(CallbackQueryHandler(handle_menus, pattern='^main_menu|gate_|verify_|recheck_|menu_(bseb|neet)|sel_sub_|mode_|tgl_|confirm_mix|sng_|time_|count_|show_help|set_grp_mode_|imp_'))
