...
# NOTE: Bot MUST be Admin in these channels/groups
MAIN_CHANNEL_USERNAME = "@errorkid_05" 

# Group IDs for API Checking (Live Verification)
...
async def check_membership(chat_id, user_id, context):
    """
    Checks if a user is a member of a specific chat via API.
    Returns True if member/admin/creator, False if left/kicked or bot not admin.
    """
    try:
        ...
        logging.error(f"Verify Fail for {chat_id}: {e}")
        # If bot is not admin, we cannot verify. 
        # Ideally return False to force join, but if config is wrong it blocks everyone.
        # Returning False here to ensure strict security as requested.
        return False 
    except Exception:
        return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Main Channel Verification (Entry Gate)
    is_joined = await check_membership(MAIN_CHANNEL_USERNAME, user_id, context)
    if not is_joined:
...
async def handle_menus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Recheck Main Channel
    if data == 'recheck_main':
        if await check_membership(MAIN_CHANNEL_USERNAME, user_id, context):
            await show_main_menu(update, context)
        else:
            await query.answer("❌ You are NOT in the channel!", show_alert=True)
        return

    # 2. BSEB GATE (Live Check)
    if data == 'gate_bseb' or data == 'recheck_bseb':
        is_member = await check_membership(ID_BSEB_GROUP, user_id, context)
        if is_member:
            await open_bseb_menu(query)
        else:
            if data == 'recheck_bseb': await query.answer("❌ Join First!", show_alert=True)
            else: await send_group_gate(query, LINK_BSEB_GROUP, "recheck_bseb")
        return

    # 3. NEET GATE (Live Check)
    if data == 'gate_neet' or data == 'recheck_neet':
        is_member = await check_membership(ID_NEET_GROUP, user_id, context)
        if is_member:
            await open_neet_menu(query)
        else:
            if data == 'recheck_neet': await query.answer("❌ Join First!", show_alert=True)
...
async def send_force_join_msg(update, name, link, cb):
    btns = [[InlineKeyboardButton("🚀 Join Channel", url=link), InlineKeyboardButton("✅ Joined", callback_data=cb)]]
...
async def send_group_gate(query, link, cb):
    ...
    await safe_edit_message(query, "⚠️ **Verification Required!**\n\nYou must be a member of our specific group to access this section.", InlineKeyboardMarkup(btns))
...
async def run_quiz_sequence(context: ContextTypes.DEFAULT_TYPE):
    ...
    save_db(db)
    await send_main_menu_direct(context, chat_id, user_id) # AUTO MAIN MENU

async def send_main_menu_direct(context, chat_id, user_id):
    """Sends Main Menu after quiz"""
    buttons = [
    ...
    try: await context.bot.send_message(chat_id, "🏠 **Main Menu:**", reply_markup=InlineKeyboardMarkup(buttons), parse_mode='Markdown')
    except: pass
...
async def handle_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_admin':
        ...
        save_db(db)
        # Update context target to reload same menu
        context.user_data['target_grp'] = gid
        # Recursive call to reload UI
        query.data = f"mng_grp_{gid}"
...
async def auto_group_job(context):
    ...
    # Placeholder for group random question logic
