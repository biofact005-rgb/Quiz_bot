...
# ==========================================
# 5. CORE QUIZ ENGINE (UPDATED FOR STOP)
# ==========================================
async def run_quiz_sequence(context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'futures' not in context.user_data: 
        context.user_data['futures'] = {}
        
    # Mark quiz as active
    context.user_data['quiz_active'] = True

    for i, q in enumerate(job_data['q']):
        # [CHECK STOP FLAG FROM USER DATA] - The main fix
        if context.user_data.get('stop_quiz_flag'): 
            ...
            context.user_data['stop_quiz_flag'] = False # Reset flag
        ...
        try:
            ...
            try: 
                await asyncio.wait_for(future, timeout=job_data['t'] + 2)
            except asyncio.TimeoutError: 
                pass
            except asyncio.CancelledError:
                # Cancelled by /done command immediately
                pass
...
async def start_private_quiz(query, context):
    ...
    # [RESET FLAGS] Ensure fresh start
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if "current_polls" in db and poll_id in db["current_polls"]:
            ...
            if sub not in db["stats"][uid_str][cat]: 
                db["stats"][uid_str][cat][sub] = {'total':0, 'correct':0, 'wrong':0}
            ...
            if selected == corr:
                ...
                if mode == 'normal':
                    ...
                    is_duplicate = any(mq['question'] == q['question'] for mq in mistakes_list)
                    if not is_duplicate:
                        mistakes_list.append(q)
...
async def show_main_menu(update, context):
    ...
    if user_id == OWNER_ID:
        btns.append([InlineKeyboardButton("👑 Owner Panel", callback_data='menu_owner')])
    if is_admin(user_id):
        btns.append([InlineKeyboardButton("🛡️ Content Admin", callback_data='menu_admin')])
...
async def handle_admin(update, context):
    ...
    if data.startswith('del_sel_'):
        cat = data.split('_')[2]; context.user_data['del_cat'] = cat
        subs = db["questions"].get(cat, {}).keys()
        btns = [[InlineKeyboardButton(s, callback_data=f'del_sub_{s}')] for s in subs]
        btns.append([InlineKeyboardButton("Back", callback_data='adm_del_menu')])
        await safe_edit_message(query, f"🗑️ <b>{cat} > Select Subject:</b>", InlineKeyboardMarkup(btns))
        return
    if data.startswith('del_sub_'):
        sub = data.replace('del_sub_', ''); context.user_data['del_sub'] = sub
    ...
    if data.startswith('adm_sel_'):
        cat = data.split('_')[2]; context.user_data['adm_cat'] = cat
        subs = db["questions"][cat].keys()
        btns = [[InlineKeyboardButton(s, callback_data=f'adm_sub_{s}')] for s in subs]
    ...
    if data.startswith('adm_sub_'):
        sub = data.split('_')[2]; context.user_data['adm_sub'] = sub
        cat = context.user_data.get('adm_cat')
        chaps = db["questions"][cat].get(sub, {}).keys()
...
async def master_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main_menu': await show_main_menu(update, context)
        ...
        mistakes = get_mistake_questions(query.from_user.id, cat, sub)
        if not mistakes:
            await query.answer("🎉 No mistakes in this subject!", show_alert=True)
            return

        context.user_data['quiz_cat'] = cat
        context.user_data['quiz_sub'] = sub
        context.user_data['quiz_mode'] = 'improve' 
        await ask_time(query)

    elif data.startswith('sel_sub_'):
        parts = data.split('_', 3); cat = parts[2]; sub = parts[3]
        context.user_data['quiz_cat'] = cat; context.user_data['quiz_sub'] = sub
        context.user_data['quiz_mode'] = 'normal'
        ...
        await safe_edit_message(query, f"📂 <b>{cat} > {sub}</b>", InlineKeyboardMarkup(btns))
...
async def open_bseb_menu(query, context):
    btns = [[InlineKeyboardButton(f"📖 {s}", callback_data=f'sel_sub_BSEB_{s}')] for s in ["Hindi", "English", "Maths", "Biology", "Chemistry", "Physics"]]
...
async def show_chapter_selection(query, context, multi):
    ...
    chapters = db["questions"].get(cat, {}).get(sub, {})
    if not chapters: await query.answer("No chapters found!", show_alert=True); return
    btns = []
    ...
    await safe_edit_message(query, f"📖 <b>Select Chapter ({sub})</b>", InlineKeyboardMarkup(btns))
...
async def ask_count(query):
    ...
    btns = [
        [InlineKeyboardButton(f"{c} Qs", callback_data=f"count_{c}") for c in counts[i:i+3]]
...
# ==========================================
# 9. COMMAND HANDLERS (UPDATED)
# ==========================================
async def done_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # [FIX] Ab hum User Data use kar rahe hain, Job Queue nahi
    if not context.user_data.get('quiz_active'):
        await update.message.reply_text("⚠️ Koi quiz chal nahi raha hai.")
        # Agar galti se bot restart hua aur state lost ho gaya toh safe side menu dikha dete hain
    ...
    # Wait loop todne ke liye Future cancel karo
    if 'futures' in context.user_data:
        for poll_id, future in context.user_data['futures'].items():
            if not future.done():
                future.cancel() # Future cancel karne se Loop mein 'CancelledError' aayega aur wo agla question nahi bhejega
    
    # Note: Hum yahan menu nahi bhej rahe, kyunki 'run_quiz_sequence' loop flag detect karke menu bhej dega
...
async def remove_admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if target_id in db["admins"]:
            ...
            await update.message.reply_text("⚠️ Ye user admin list me nahi hai.")
...
if __name__ == '__main__':
    ...
    if not TOKEN: print("❌ TOKEN MISSING")
        ...
        app.add_handler(CommandHandler("done", done_command))        # [NEW] Replaces stop
        app.add_handler(CommandHandler("removeadmin", remove_admin_command)) # [NEW] Remove Admin
