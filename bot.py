...
# --- SAFE EDIT MESSAGE (Prevents "Message Not Modified" Error) ---
...
async def send_join_prompt(update, link, callback_success):
    ...
    if update.callback_query:
        await safe_edit_message(update.callback_query, text, InlineKeyboardButton(btns))
    else:
...
async def start_private_quiz(query, context):
    ...
    await safe_edit_message(query, msg_text, None) # No markup to remove buttons
...
async def handle_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_admin':
        ...
        await safe_edit_message(query, "⌨️ **Type the Interval in seconds** (e.g. 120, 600):", None)
        context.user_data['awaiting_interval'] = True
    
    # Owner Commands (Add Admin / Backup)
    elif data == 'add_admin_prompt':
        await safe_edit_message(query, "🆔 **Send the User ID** to promote to Admin.", None)
        ...
        await safe_edit_message(query, "📤 **Send the `backup.json` file now** to restore.", None)
    ...
    # Add Questions
        ...
        await safe_edit_message(query, "⌨️ **Type Chapter Name:**", None)
...
async def show_settings(update, context):
    ...
    # Fixed: Buttons are added to a list properly
...
async def auto_group_job(context):
    ...
    # Random Fetch Logic (Placeholder)
...
if __name__ == '__main__':
    ...
    if not TOKEN: print("❌ TOKEN MISSING")
        ...
        app.add_handler(MessageHandler(filters.Document.MimeType("application/json"), handle_file_upload)) # Restore handler
