...
# ==========================================
def load_db():
    # [UPDATED] Structure for BSEB Sections
    default_db = {
        "questions": {
            "BSEB": {
                # Hindi Sections
    ...
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, 'r') as f:
                data = json.load(f)
                # Merge logic to ensure new keys exist if old DB is loaded
                for k, v in default_db.items():
                    if k not in data: 
                        data[k] = v
                    elif isinstance(v, dict) and k == "questions":
                        for cat, subs in v.items():
                            if cat not in data["questions"]:
                                data["questions"][cat] = subs
                            else:
                                for sub_key, sub_val in subs.items():
                                    if sub_key not in data["questions"][cat]:
                                        data["questions"][cat][sub_key] = sub_val
                return data
        except: pass
    return default_db
...
def get_mistake_questions(user_id, category, subject):
    try:
        ...
        # [NEW LOGIC] Handle Combined Sections for Improve
        targets = []
...
async def start_private_quiz(query, context):
    ...
    if mode == 'improve':
        ...
        # [NEW LOGIC] Handle Sub-Section Selection
        # Note: 'sub' here will be like 'Hindi-Gadya' or 'Maths'
        chaps = context.user_data.get('final_chapters')
        questions = get_random_questions(cat, sub, chaps, req_count)
        
        disp_sub = sub.split('-')[-1] if '-' in sub else sub # Display nice name
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if "current_polls" in db and poll_id in db["current_polls"]:
            ...
            # Use specific subject key for stats (e.g., Hindi-Gadya)
...
async def handle_admin(update, context):
    ...
    if data == 'menu_admin':
        # Admin menu uses real DB keys (Hindi-Gadya etc)
        btns = [
    ...
    # [ADMIN] Generic handlers... (unchanged logic)
    ...
    if data.startswith(('del_sel_', 'adm_sel_')):
        ...
        # Admin sees raw names (Hindi-Gadya) to know exactly where adding
...
async def master_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'main_menu': await show_main_menu(update, context)
        ...
        # [MODIFIED] Custom logic for BSEB Improve menu
        ...
        cat = data.split('_')[2]
        # [NEW] Manual list for BSEB Improve to hide sub-sections
        if cat == 'BSEB':
    ...
    # [NEW HANDLERS FOR SECTIONS]
    elif data == 'section_BSEB_Hindi':
        await open_bseb_hindi_sections(query, context)
    elif data == 'section_BSEB_English':
        await open_bseb_english_sections(query, context)
        ...
        disp_sub = sub.split('-')[-1] # Clean name for display
...
async def open_bseb_menu(query, context):
    # [UPDATED] Uses Sub-Menus for Hindi/English
    btns = [
